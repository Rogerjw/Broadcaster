...
class User {
  ...
  static addUser(user) {
    return pool.query(queries.addUser, [user.firstname, user.lastname, user.email,
      user.password, user.PhoneNumber,
      user.username, user.type]);
  }

  static findUserWithUsername(username) {
    return pool.query(queries.findUserWithUsername, [username]);
  }
}
export default User;
